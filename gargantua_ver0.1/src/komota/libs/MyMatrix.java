...
public class MyMatrix{
	...
	/**
	 * 行列式を求める.Statics.MATRIX_MIN_DETERMINANT未満である場合は 0 を返す.
	 * @return この行列の行列式
	 */
	public double getDetV(){
		...
		if(output < Statics.MATRIX_MIN_DETERMINANT){
			return 0;
		}
		return output;
	}
	...
	/**
	 * 行列値を，Statics.MATRIX_MIN_APPROX未満を四捨五入して概数化する．このメソッドのみでは細かい数値が残ってしまう場合があるため，使用には注意が必要.
	 * @return 個の行列を概数化した行列
	 */
	public MyMatrix approximate(){
		...
		for(int i=0;i<this.dimension;i++){
			for(int j=0;j<this.dimension;j++){
				double temp1 = this.data[i][j] / Statics.MATRIX_MIN_APPROX;

				int temp2 = (int) Math.round(temp1);

				output.setData(i, j, (double)temp2*Statics.MATRIX_MIN_APPROX);
			}
		}
