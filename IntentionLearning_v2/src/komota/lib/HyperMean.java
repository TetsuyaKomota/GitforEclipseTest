...
public class HyperMean {
	...
	public double[] getHyperMean(){
		...
		//データ量が元のデータ量の percentage 倍にまで減ったら終了
		int resize = (int)(this.data.size() * percentage);
		while(this.data.size() > 1){
			...
			//データ量が目標値以上なら取り除いたデータを削除し，最初に戻る
			if(this.data.size() > resize){
			}
			//目標値以下なら取り除いたデータを入れ直し，ループを終了する
			else{
	...
	//枝刈り平均出力
	public double[] getHyperMean_old(){
		double [] output = new double[this.datalength];
