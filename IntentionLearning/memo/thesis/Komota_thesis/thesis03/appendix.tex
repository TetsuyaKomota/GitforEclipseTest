...
$unit$は正規化長を表す。正規化関数$F_{lk}(v)$は、教示動作から得られた参照点から目標位置までの相対位置ベクトルを、あらかじめ定めた正規化長に正規化する。これは例えば物体に近づける動作など、距離的な位置関係よりも初期位置からの変化の割合が重視されるような動作や、複数の物体と相似的な配置を目標とする動作の学習に必要になる処理である。
