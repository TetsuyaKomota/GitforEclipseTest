...
public class PR_7_1_GL extends MyPR{
	...
	ReferencePoint_Gauss[][][][][][][][][][] refs_GL;

	//空間内に、土のオブジェクトが存在するかのリスト。実装を単純にするためだけのものであり、一般的には不要
	int[] objectlist;

	//コンストラクタ
	public PR_7_1_GL(int numref,String filename){
		super(filename);
		this.numref = numref;
		...
		this.refs_GL = new ReferencePoint_Gauss[2][2][2][2][2][2][2][2][2][9];
											...
											if(isexist == true && tempnum >= 2){
												...
												for(int a=0;a<tempidx.length;a++){
													if(a != 1 && tempidx[1] == 0 && tempidx[a] == 1){
														this.refs_GL[tempidx[0]][tempidx[1]][tempidx[2]][tempidx[3]][tempidx[4]][tempidx[5]][tempidx[6]][tempidx[7]][tempidx[8]][a] = new ReferencePoint_Gauss(this,11,temppoint[0],temppoint[1]);
													}
												}
											}
										}
	...
	public void learnfromLog(){
												...
												for(tempidx[8] = 0;tempidx[8]<2;tempidx[8]++){
													for(int a=0;a<tempidx.length;a++){
														if(this.refs_GL[tempidx[0]][tempidx[1]][tempidx[2]][tempidx[3]][tempidx[4]][tempidx[5]][tempidx[6]][tempidx[7]][tempidx[8]][a] != null){
															...
															this.refs_GL[tempidx[0]][tempidx[1]][tempidx[2]][tempidx[3]][tempidx[4]][tempidx[5]][tempidx[6]][tempidx[7]][tempidx[8]][a].learnVariance(trajector,startpoint);
														}
													}
												}
											}
		...
		/* *********************************************************************************************************** */
		/* 以上で分散の計算が終了してるので、改めてlikelihoodを計算する                                                */
		/* *********************************************************************************************************** */
		for(int t=0;t<this.logdata.length;t++){
			if(logdata[t] == null){
				break;
