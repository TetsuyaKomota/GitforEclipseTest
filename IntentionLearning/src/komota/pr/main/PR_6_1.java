...
public class PR_6_1 extends MyPR{
	...
	//コンストラクタ
	public PR_6_1(int numref,String filename){
		super(filename);
		this.numref = numref;
			...
			if(this.logdata[t].getType() == GOAL){
				...
				for(int i=0;i<height;i++){
					for(int j=0;j<width;j++){
						...
						//タスク的視野の中にいることも条件
						if(Math.abs(i-tempgoal[0])<this.coordinate.getRange() && Math.abs(j-tempgoal[1])<this.coordinate.getRange() && this.logdata[0].getStepStatus(i,j) > 1){
...
*/
	...
	public void reproduction(MyFrame frame){
		...
		tempoutput = PR_6_1.this.coordinate.inverseConvert(inputs);
							...
							for(tempidx[4] = 0;tempidx[4]<2;tempidx[4]++){
								for(tempidx[5] = 0;tempidx[5]<2;tempidx[5]++){
									for(tempidx[6] = 0;tempidx[6]<2;tempidx[6]++){
										for(tempidx[7] = 0;tempidx[7]<2;tempidx[7]++){
											for(tempidx[8] = 0;tempidx[8]<2;tempidx[8]++){
												if(this.cogs[tempidx[0]][tempidx[1]][tempidx[2]][tempidx[3]][tempidx[4]][tempidx[5]][tempidx[6]][tempidx[7]][tempidx[8]] != null && this.cogs[tempidx[0]][tempidx[1]][tempidx[2]][tempidx[3]][tempidx[4]][tempidx[5]][tempidx[6]][tempidx[7]][tempidx[8]].likelihood > output){
													output = this.cogs[tempidx[0]][tempidx[1]][tempidx[2]][tempidx[3]][tempidx[4]][tempidx[5]][tempidx[6]][tempidx[7]][tempidx[8]].likelihood;
												}
											}
										}
									}
